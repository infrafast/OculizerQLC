...
class Oculizer(threading.Thread):
    def __init__(self, scene_manager, input_device='cable',
                 scene_prediction_enabled=False, predictor_version='v6',
                 average_dual_channels=False, scene_cache_size=10,
        ...
        self.audio_processing_enabled = bool(scene_prediction_enabled)
        self.device_idx = (
        ...
        self.prediction_audio_sample_rate = self.sample_rate
    ...
    def _init_scene_prediction(self):
        ...
        logger.info(
            "Scene prediction initialized with %s predictor at %dHz; "
    ...
    def prediction_processing_thread(self):
        ...
        while self.running.is_set():
            try:
                ...
                # Resample the shared analysis-rate window for the predictor.
    ...
    def audio_callback(self, indata, frames, time, status):
        ...
        # Optionally average channels 1 and 2 from the one input device.
        ...
        try:
            ...
            with SoundDeviceAudioSource(
                ...
                if self.scene_prediction_enabled:
                    self.prediction_thread = threading.Thread(target=self.prediction_processing_thread, daemon=True)
                    self.prediction_thread.start()
                    logger.info("Scene prediction uses the shared audio input")
