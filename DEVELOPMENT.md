...
- [ ] make the deployment readiness helper non-blocking for `qlc-native`: the current `ExecStartPre` waits up to 30 seconds for TCP `9998` and can prevent Oculizer from starting when QLC+ starts later, which conflicts with the accepted asynchronous strategy. In native mode, start Oculizer regardless of QLC+ availability and let the bounded reconnect loop attach when QLC+ appears; retain the existing readiness behavior for other outputs unless separately revised;
