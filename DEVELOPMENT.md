...
- [ ] validate late QLC+ startup and QLC+ restart under systemd. A new native session may require a new GUI authorization, and an idle broken connection is currently discovered on the next outbound command rather than immediately;
...
The same live session then validated QLC+ loss and recovery. Stopping QLC+ caused the native client to detect refusal, clear its session inventory, enter `disconnected`, and retry at the configured two-second cadence while audio analysis and prediction continued. After QLC+ restarted, the unchanged Oculizer process connected to a new native session, waited for fresh operator authorization, downloaded the current project again, rebuilt its widget inventory, and returned to `ready`. Remaining observability work: repeated `connecting`/`disconnected` state messages currently appear on every retry and must be rate-limited or summarized without reducing the actual reconnect cadence.
